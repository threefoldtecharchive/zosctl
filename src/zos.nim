...
proc newContainer(this:App, name:string, root:string, hostname="", privileged=false, timeout=30, sshkey="", ports="", env="", zerotier=""):int =
  ...
  if not extraArgs.hasKey("nics"):
    if zerotier == "":
      ...
      extraArgs["nics"] = %*[ %*{"type": "default"}, %*{"type": "zerotier", "id":zerotier}]
...
proc handleConfigured(args:Table[string, Value]) = 
  ...
  proc handleContainerNew() =
    ...
    let containerId = app.newContainer(containername, rootflist, hostname, privileged, sshkey=sshkey, ports=ports, env=env, zerotier=zerotier)
