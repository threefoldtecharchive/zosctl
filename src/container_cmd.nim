...
proc stopContainer(name, host: string, port: int) :int = 
  echo name, host, $port
  result = 1 

proc deleteContainer(name, host: string, port: int): int = 
  echo name, host, $port

  result = 1

proc sandboxContainer(name, host: string, port: int) :int = 
  echo name, host, $port

  result = 1

proc listContainers(host: string, port: int):int = 
  echo  host, $port
  result = 1


proc prepareZeroOSLocally(vboxMachineName: string): int = 
...
when isMainModule:
  import cligen
  dispatchMulti([startContainer], [stopContainer], [listContainers], [sandboxContainer], [deleteContainer], [prepareZeroOSLocally])
